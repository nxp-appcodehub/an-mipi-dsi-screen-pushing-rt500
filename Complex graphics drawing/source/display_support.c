...
dsi_dphy_config_t mipiDphyConfig;
...
#if (HIJACK_MIPI_TE_PIN == 1)
    GPIO_PinInit(GPIO, BOARD_MIPI_TE_PORT, BOARD_MIPI_TE_PIN, &tePinConfig);

    RESET_PeripheralReset(kINPUTMUX_RST_SHIFT_RSTn);
    INPUTMUX_Init(INPUTMUX);
    INPUTMUX_AttachSignal(INPUTMUX, kPINT_PinInt0, kINPUTMUX_GpioPort0Pin19ToPintsel);
    INPUTMUX_Deinit(INPUTMUX);
...
#else
    gpio_interrupt_config_t tePinIntConfig = {kGPIO_PinIntEnableLevel, kGPIO_PinIntEnableHighOrRise};
...
#else
    gpio_interrupt_config_t tePinIntConfig;
    tePinIntConfig.mode = kGPIO_PinIntEnableLevel;

    if(GPIO_PinRead(GPIO, BOARD_MIPI_TE_PORT, BOARD_MIPI_TE_PIN) == 1) {
        tePinIntConfig.polarity = kGPIO_PinIntEnableLowOrFall;		
        GPIO_SetPinInterruptConfig(GPIO, BOARD_MIPI_TE_PORT, BOARD_MIPI_TE_PIN, &tePinIntConfig);
//#ifdef GPIO_DEBUG
...
//#endif
        ...
        tePinIntConfig.polarity = kGPIO_PinIntEnableHighOrRise;		
        GPIO_SetPinInterruptConfig(GPIO, BOARD_MIPI_TE_PORT, BOARD_MIPI_TE_PIN, &tePinIntConfig);
    }
#endif
}
...
static void BOARD_SetMipiDsiConfig(void)
{
    ...
    DSI_GetDphyDefaultConfig(&mipiDphyConfig, mipiDsiDphyBitClkFreq_Hz, mipiDsiTxEscClkFreq_Hz);

    /* Init the DSI module. */
    DSI_Init(DEMO_MIPI_DSI, &dsiConfig);

    /* Init DPHY. There is not DPHY PLL, the ref clock is not used. */
    DSI_InitDphy(DEMO_MIPI_DSI, &mipiDphyConfig, 0);
}
